...
public struct CameraView: View {
    ...
    @EnvironmentObject var mediaPickerViewModel: MediaPickerViewModel
    
    let showPreviewClosure: () -> Void
    let didTakePicture: () -> Void
    let didStartVideoRecording: () -> Void
    let didStopVideoRecording: () -> Void
    ...
    public var body: some View {
        UIKitCameraView(
            onPhotoCapture: { url in
                print("📸 Photo captured at: \(url)")
                // Set the URL in the MediaPickerViewModel
                mediaPickerViewModel.pickedMediaUrl = url
                
                // Call the existing closure to handle the photo
                didTakePicture()
